...
var fetchData = function () {
  ...
  return fetch('https://api.npmjs.org/downloads/range/' + yearago + ':' + yesterday + '/webfly').then(function (
    response) {
...
var plot = function (npmData) {
  ...
  colorScale = (new Plottable.Scale.Color()).domain(["daily webfly installs"]).range(["#EF5734"]);
...
function fetchResults(query, options) {
  ...
  return fetch('https://libraries.io/api/webfly-search?q=' + query).then(function (results) {
    return results.json();
  });
}
